    ...
    <div className="fixed inset-0 bg-white z-[60] flex flex-col animate-in fade-in slide-in-from-bottom duration-300">
      <header className="bg-white border-b px-6 py-5 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-5">
          <button onClick={onClose} className="w-12 h-12 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 active:scale-90 transition-all shadow-sm">
        ...
        <button onClick={() => geminiService.speak("Support bridge established.")} className="w-12 h-12 rounded-2xl bg-gray-50 text-gray-400 flex items-center justify-center hover:bg-green-50 hover:text-green-600 transition-all shadow-sm">
      ...
      <div className="bg-gray-900 text-white px-8 py-8 border-b border-gray-800">
        ...
        <div className="relative h-2 w-full bg-gray-800 rounded-full overflow-hidden">
          <div 
            className={`absolute top-0 left-0 h-full transition-all duration-1000 ease-out shadow-[0_0_12px_rgba(34,197,94,0.3)] ${sentiment.bg}`}
            style={{ width: `${Math.min(100, Math.max(10, 50 + Number(calculateDiff()) * 2))}%` }}
          ></div>
        </div>
      </div>
                    ...
                    <div className="p-6">
                       ...
                       {!isMe && (
                        <div className="grid grid-cols-2 gap-3 mt-6">
                           <button onClick={() => handleAction('ACCEPT')} className="bg-emerald-600 text-white py-3.5 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] shadow-lg shadow-emerald-100 active:scale-95 transition-all">Accept</button>
      ...
      <div className="p-6 bg-white border-t pb-10">
        ...
        <div className="flex justify-center gap-10 mt-6">
           <button onClick={() => handleAction('ACCEPT')} className="text-[10px] font-black uppercase tracking-[0.25em] text-emerald-600 hover:text-emerald-700 transition-colors">Accept Deal</button>
